...
export class WaterGun extends Attack {
    constructor() {
        super("Water Gun", undefined, 20, 0, ["water", "normal"], [1, 0], 0, "");
    }
    attackAction(defender, attacker) {
        ...
        console.log(`Water Gun deals ${totalDamage} damage to ${defender.pokemonName}`);
        return true;
    }
}
export class WaveSplash extends Attack {
    constructor() {
        super("Wave Splash", undefined, 40, 0, ["water", "normal"], [1, 1], 0, "");
    }
    attackAction(defender, attacker) {
        if (!attacker.checkEnergyRequirements(this.EnergyType, this.EnergyCost)) {
        ...
        let totalDamage = this.applyWeaknessBonus(defender, this.Damage);
        defender.applyDamage(totalDamage);
        console.log(`Wave Splash deals ${totalDamage} damage to ${defender.pokemonName}`);
        return true;
    }
}
export class HydroPump extends Attack {
    constructor() {
        super("Hydro Pump", undefined, 80, 0, ["water", "normal"], [2, 1], 0, "");
    }
    extraDamage(attacker) {
    ...
    attackAction(defender, attacker) {
        ...
        defender.applyDamage(totalDamage);
        console.log(`Hydro Pump deals ${totalDamage} damage to ${defender.pokemonName}`);
        return true;
    }
}
export class Surf extends Attack {
    ...
    attackAction(defender, attacker) {
        ...
        let totalDamage = this.applyWeaknessBonus(defender, this.Damage);
        defender.applyDamage(totalDamage);
        console.log(`Surf deals ${totalDamage} damage to ${defender.pokemonName}`);
        return true;
    }
}
export class HydroBazooka extends Attack {
    constructor() {
        super("Hydro Bazooka", undefined, 100, 0, ["water", "normal"], [2, 1], 0, "");
    }
    extraDamage(attacker) {
    ...
    attackAction(defender, attacker) {
        ...
        totalDamage = this.applyWeaknessBonus(defender, totalDamage);
        defender.applyDamage(totalDamage);
        console.log(`Hydro Bazooka deals ${totalDamage} damage to ${defender.pokemonName}`);
        return true;
    }
}
//# sourceMappingURL=WaterAttacksClass.js.map